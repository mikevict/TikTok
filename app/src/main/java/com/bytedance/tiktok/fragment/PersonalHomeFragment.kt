...
class PersonalHomeFragment : BaseBindingFragment<FragmentPersonalHomeBinding>({FragmentPersonalHomeBinding.inflate(it)})
    ...
    fun init() {
        ...
        binding.homeHeader.tvAddfocus!!.setOnClickListener(this)
        setUserInfo()
    }
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.tvAddfocus -> {
                // 关注/取消关注逻辑
                if (curUserBean != null) {
                    curUserBean!!.isFocused = !curUserBean!!.isFocused
                    updateFollowStatus()
                }
            }
        }
    ...
    private fun updateFollowStatus() {
        if (curUserBean != null) {
            ...
            // 更新DataCreate中所有对应用户视频的关注状态，确保播放页面加号正确显示/隐藏
            updateVideoFocusStatus()
        }
    }
     ...
     */
    private fun updateVideoFocusStatus() {
        if (curUserBean != null) {
            // 遍历所有视频，找到对应用户的视频，更新关注状态
            for (videoBean in DataCreate.datas) {
                if (videoBean.userBean != null && videoBean.userBean!!.uid == curUserBean!!.uid) {
                    videoBean.isFocused = curUserBean!!.isFocused
                }
            }
        }
    }
